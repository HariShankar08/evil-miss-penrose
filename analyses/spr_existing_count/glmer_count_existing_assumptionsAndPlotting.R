...
plot_folder = ".../spr_existing_count/models_assumptions_plots/plot_assumptions_BERT_original/" # path to folder where plots are saved 
...
plot_folder = ".../spr_existing_count/models_assumptions_plots/plot_assumptions_llama_original/"
model_folder = ".../spr_existing_count/results_llama_original/"
zeroinflation <- dispersion <- list()
...
plot_folder = ".../spr_existing_count/models_assumptions_plots/plot_assumptions_baseline_models"
model_folder = ".../spr_existing_count/results_baseline"
...
plot_folder = ".../spr_existing_count/models_plots/"

# ______________________________________________________________________________
# (2.1) plot by-layer results - all models -------------------------------------

fold = ".../spr_existing_count" # path to model files (.rda)
...
png(paste(plot_folder,"spr_Existing_byLayer.png",sep = ""), height = 16, width = 24, units = "cm", res = 1000)
...
plot(rep(res_bas$beta[1],40), pch = 21, col = "darkslategray4", type = "o", lwd = 1.5,
     ...
     main = "Contextualized existing compounds: by-layer predictability",
...
axis(side = 2, at = round(seq(-0.15,0.65, by = 0.05),2), cex.axis = 0.8)

legend("topright", c("BERT: original relations",
...
png(paste(plot_folder,"spr_Existing_byLayer_AIC.png",sep = ""), height = 16, width = 24, units = "cm", res = 1000)
...
plot(rep(res_bas$AIC[1],40), col = "darkslategray4", type = "o", lwd = 1.5,
     ...
     main = "Contextualized existing compounds: by-layer model fit",
...
abline(h = c(round(seq(9025,9275, by = 25),2)), col = rgb(0,0,0,alpha = 0.05))
abline(v = c(0:41), col = rgb(0,0,0,alpha = 0.05))

axis(side = 1, at = seq(1,40,by = 3), cex.axis = 0.8)
axis(side = 2, at = round(seq(9025,9275, by = 25),2), cex.axis = 0.8)
legend("bottomright", c("BERT: original relations",
                  "BERT: alternative wording",
                  "LLaMA-2-13B: original relations",
                  "LLaMA-2-13B: alternative wording",
                  "baseline: vector difference",
...
load(".../spr_existing_count/results_BERT_original/negBinomial_BERT_original_L12.rda")
...
ggsave(filename = "existing_multiPlot_BERT.jpeg", 
       path = plot_folder,
...
load(".../spr_existing_count/results_llama_original/negBinomial_llama_original_L14.rda")

sjPlot::plot_model(m, type = "pred",
                   terms = c("p_llama_original_L14", "relation"),
                   theme_classic(),
                   show.data = T,
                   pred.type = "re", # suppress to plot fixed main effect 
                   colors = mycolors[1:16],
  ...
  facet_wrap(~group) + # suppress to plot in one panel
  labs(y = "human selection frequency",
       x = "LLaMA-2-13B relation probability (layer 14)")

ggsave(filename = "existing_multiPlot_llama.jpeg", 
       path = plot_folder,
       width = 20, height = 15, units = "cm", device='jpeg', dpi=1000)
# single plot size: width = 15, height = 15
